...
> **개정(2026-08-16, [ADR-0039](./0039-listing-schema-v4-registration-form.md) Amended)**: §6의 예외를 `Listing.contact{managerName, phone}` **둘로 좁힌다.** 위 2026-08-12 개정은 셋(`managerName`·`phone`·`sms`)을 예외로 뒀지만, 그 근거인 "`users.phone_number`와 별개 값"이 **`sms`에서만은 성립하지 않았다** — 문자문의 번호 칸에 적히는 값은 임대인이 온보딩에서 SMS 인증을 통과한 바로 그 번호 자신이라, §6이 마스킹 대상으로 정한 PII를 매물 응답 경로로 평문 공개하는 통로였다. 그래서 [ADR-0039](./0039-listing-schema-v4-registration-form.md) Amended가 `sms` 필드를 제거했고, 이에 맞춰 §6에 **임대인 개인 연락처를 매물 문서에 복사하지 않는다**는 원칙을 함께 적는다 — 그 번호가 실제로 필요해지면 저장된 사본이 아니라 **조회 시점에 `user :: api`로 가져와 이 §6의 마스킹 규칙을 그대로 적용**한다([ADR-0047](./0047-web-local-credentials-and-phone-based-account-linking.md)이 `users.phone_number`에 UNIQUE를 걸어 이 번호가 계정 매칭의 유일한 키가 된 뒤로는 사본이 늘수록 위험만 커진다). **`users.phone_number`의 SMS 인증·마스킹 결정도, 남은 `contact{managerName, phone}`(지점 대표 전화)의 평문 공개도 그대로 유지**된다 — 변경은 예외 목록에서 `sms`가 빠진 것에 한정된다.
...
6. **연락처 저장·변경**: 검증 통과한 `phoneNumber`를 `users.phone_number`로 영속한다(임대인 전용, 세입자 NULL). 응답·로그·`toString`에는 **마스킹**해 노출한다(예 `010-****-5678`). 본인 `GET /users/me`만 평문 반환. **프로필에서 연락처를 변경할 때(US-1-5)도 새 번호를 SMS 재인증해 VERIFIED된 뒤에만 반영**하며(미인증·불일치 `422 AUTH_PHONE_NOT_VERIFIED`), 검증 엔드포인트(`/auth/phone/verification-code`·`/auth/phone/verify`)와 마커를 그대로 재사용한다 — 이때는 정식 토큰(`ACTIVE`) 컨텍스트를 허용하도록 보안 경로 티어를 확장한다. **예외**: 매물 문서의 `Listing.contact{managerName, phone}`는 `users.phone_number`와 **별개 값**이다 — 임대인이 매물별로 입력하는 담당 연락처(`phone`은 지점 대표 전화)이므로 SMS 인증 대상도 마스킹 대상도 아니며, **세입자 응답에 평문으로 공개**된다([ADR-0039](./0039-listing-schema-v4-registration-form.md)). 반면 이 예외가 성립하지 않던 `contact.sms`(임대인이 인증한 개인 번호를 그대로 적게 되는 칸)는 **제거됐다** — 개인 번호는 매물 문서에 복사하지 않고, 필요하면 조회 시점에 `user :: api`로 가져와 이 §6의 마스킹 규칙을 그대로 적용한다([ADR-0039](./0039-listing-schema-v4-registration-form.md) Amended).
