...
sequenceDiagram
    ...
    alt 온보딩 스코프 토큰(ROLE_ONBOARDING)
        ...
        alt userType=TENANT (세입자)
            ...
            Note over USER: name=FullName.firstName(전체 이름, lastName 미사용)<br/>세입자 전용 필드(gender/country/countryName/countryFlag/<br/>occupation/visaType)·email 미포함(임대인 이메일 미수집)<br/>birthDate 포함(임대인도 온보딩 수집)·businessRegistrationNumber 미포함(원문 비저장)
            USER-->>C: 200 OK<br/>{ success, data: { id, userType=LANDLORD, name, nickname, birthDate,<br/>phoneNumber, status, termsOfServiceAgreed,<br/>privacyPolicyAgreed, marketingAgreed, createdAt }, error }
        end
        C-->>U: 프로필 표시
    end
...
- 응답 필드는 `userType`에 따라 갈린다 — **세입자(TENANT)** 는 위의 성별·국적·직업·비자 등 세입자 전용 필드를 포함하고, **임대인(LANDLORD)** 은 `userType=LANDLORD`·`name`(=`FullName.firstName`, `lastName` 미사용)·`birthDate`·`phoneNumber`·`status`·약관 동의·`createdAt`을 포함한다(세입자 전용 필드 `gender`/`country`/`occupation`/`visaType` 및 `email`(임대인 미수집)·`businessRegistrationNumber`는 미포함이나 `birthDate`는 임대인도 수집·반환, 사업자 검증은 원문 비저장).
